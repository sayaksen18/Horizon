...
export const generateQuestion = async (req, res) => {
  try{
    ...
    if(user.credits < 50){
      return res.status(400).json({message:"Insufficient credits to generate question"});
    }
    const projectText = Array.isArray(projects) && projects.length ? skills.join(", ") : "None";
...
`
    ...
    question.feedback = parsed.feedback || "No feedback provided.";
    await interview.save();
